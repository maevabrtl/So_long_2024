...
t_map	content_is_valid(int fd, t_map map)
{
	...
	if (!line)
		clean_and_exit(VOID_MAP, 3, "%C %C2 %F", line, map.map, fd);
	map.width = ft_strlen(line) - 1;
	if (map.width <= 2)
		clean_and_exit(INVALID_ELEM, 3, "%C %C2 %F", line, map.map, fd);
	map.height = 0;
	while (line[index] == WALL)
		index++;
	if (index != map.width)
		clean_and_exit(NOT_RECT, 3, "%C %C2 %F", line, map.map, fd);
	while (line)
	{
		map.height++;
		free(line);
...
size_t	is_closed_and_rectangular(t_map map)
{
	...
	if (map.height == map.width)
		clean_and_exit(NOT_RECT, 1, "%C2", map.map);
	y = -1;
	while (++y < map.height)
		if (map.map[y][map.width - 1] != WALL
			|| map.map[y][0] != WALL)
			clean_and_exit(NOT_CLOSED, 1, "%C2", map.map);
	y--;
	x = -1;
	while (++x < map.width - 1)
		if (map.map[y][x] != WALL)
